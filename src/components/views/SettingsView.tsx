      ...
      <section className="p-6 rounded-2xl bg-card-dark border-2 border-secondary/40 flex flex-col gap-4 shadow-lg">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base font-bold text-on-surface">Candidate Account</h2>
                <span
              ...
              <p className="text-xs text-text-muted">
                {userStats.authProvider === 'google' || userStats.authProvider === 'email'
                  ? `Signed in as ${userStats.name} (${userStats.email || 'Candidate'})`
                  : 'Sign in or create an account to sync your study streak, XP, and queue across devices.'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 pt-1">
          {userStats.authProvider === 'google' || userStats.authProvider === 'email' ? (
            ...
            <>
              <button
                onClick={() => (onOpenAuth ? onOpenAuth('signup') : handleGoogleSignIn())}
                className="px-5 py-2.5 bg-primary text-on-primary-fixed rounded-xl text-xs uppercase font-extrabold tracking-wider btn-pressable-primary flex items-center gap-2 shadow-sm"
              >
              ...
              <button
                onClick={() => (onOpenAuth ? onOpenAuth('signin') : handleGoogleSignIn())}
                className="px-4 py-2.5 bg-surface-container hover:bg-surface-variant text-on-surface rounded-xl text-xs uppercase font-bold tracking-wider border border-card-border flex items-center gap-2 transition-colors"
              >
              ...
              <button
                ...
                disabled={authLoading}
                className="px-4 py-2.5 bg-surface-container hover:bg-surface-variant text-on-surface rounded-xl text-xs uppercase font-bold tracking-wider border border-card-border shadow-sm flex items-center gap-2 transition-all active:translate-y-0.5"
              >
                ...
                <span>{authLoading ? 'Connecting...' : 'Google'}</span>
              </button>
            </>
          )}
        </div>
